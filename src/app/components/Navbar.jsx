...
function Navbar() {
  ...
  return (
    <nav className="md:px-[90px] px-[50px] flex items-end justify-between pt-3">
      <div className="logo flex items-end gap-2">
        <Image src="/icon.png" alt="icon app" width={40} height={40} />
        <h1 className="text-1xl font-bold">BunBounty</h1>
      </div>
      <div className="">
        <ul className="hidden md:flex">
          {links.map(({ id, link }) => (
            <li key={id} className="nav-links px-4 cursor-pointer capitalize text-[14px] text-gray-500 hover:scale-105 hover:text-gary-900 duration-200 link-underline">
              <Link href={link}>{link}</Link>
            </li>
          ))}
        </ul>
      </div>
      <div className="hidden md:flex items-center gap-2 border-[1px] border-gray-300 rounded-full">
        ...
        <div className="pr-3">
          <p className="text-[13px]">Cart (0)</p>
        </div>
      </div>
